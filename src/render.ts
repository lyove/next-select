...
export interface Callbacks {
  ...
  addable?: (
    value: string,
  ) => Promise<OptionOptional | string> | OptionOptional | string | false | undefined | null;
...
export default class Render {
  ...
  public classes = {
    // Main
    main: "next-select-main",

    // Placeholder
    placeholder: "next-select-placeholder",

    // Values
    values: "next-select-values",
    single: "next-select-single",
    max: "next-select-max",
    value: "next-select-value",
    valueText: "next-select-value-text",
    valueDelete: "next-select-value-delete",
    valueOut: "next-select-value-out",

    // Deselect
    deselect: "next-select-deselect",
    deselectPath: "M10,10 L90,90 M10,90 L90,10", // Not a class but whatever

    // Arrow
    arrow: "next-select-arrow",
    arrowClose: "M10,30 L50,70 L90,30", // Not a class but whatever
    arrowOpen: "M10,70 L50,30 L90,70", // Not a class but whatever

    // Content
    content: "next-select-content",
    openAbove: "next-select-open-above",
    openBelow: "next-select-open-below",

    // Search
    search: "next-select-search",
    searchHighlighter: "next-select-search-highlight",
    searching: "next-select-searching",
    addable: "next-select-addable",
    addablePath: "M50,10 L50,90 M10,50 L90,50", // Not a class but whatever

    // List optgroups/options
    list: "next-select-list",

    // Optgroup
    optgroup: "next-select-optgroup",
    optgroupLabel: "next-select-optgroup-label",
    optgroupLabelText: "next-select-optgroup-label-text",
    optgroupActions: "next-select-optgroup-actions",
    optgroupSelectAll: "next-select-selectall", // optgroup select all
    ...
    optgroupClosable: "next-select-closable",

    // Option
    option: "next-select-option",
    optionDelete: "M10,10 L90,90 M10,90 L90,10", // Not a class but whatever
    highlighted: "next-select-highlighted",

    // Misc
    open: "next-select-open",
    close: "next-select-close",
    selected: "next-select-selected",
    error: "next-select-error",
    disabled: "next-select-disabled",
    hide: "next-select-hide",
  };
  ...
  public updateClassStyles(): void {
    ...
    if (this.settings.contentPosition === "relative") {
      this.content.main.classList.add("next-select-" + this.settings.contentPosition);
    }
  }
  ...
  private renderMultipleValues(): void {
    ...
    const selectedOptions = this.store.filter((opt: Option) => {
    ...
    const removeNodes: HTMLDivElement[] = [];
    ...
    if (this.callbacks.addable) {
      ...
      addable.onclick = (e: Event) => {
        ...
        const runFinish = (oo: OptionOptional) => {
          ...
          if (this.settings.isMultiple) {
            const values = this.store.getSelected();
  ...
  public highlight(dir: "up" | "down"): void {
    ...
    for (let i = 0; i < options.length; i++) {
      if (options[i].classList.contains(this.classes.highlighted)) {
        ...
        // If previous option has parent classes next-select-optgroup with next-select-open then click it
        ...
        // If selected option has parent classes next-select-optgroup with next-select-close then click it
  ...
  public moveContentBelow(): void {
    ...
    if (this.settings.contentPosition !== "relative") {
      this.content.main.style.top =
        containerRect.top + containerRect.height + window.scrollY + "px";
      this.content.main.style.left = containerRect.left + window.scrollX + "px";
      this.content.main.style.width = containerRect.width + "px";
    }
  }
