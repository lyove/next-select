...
export interface Events {
  ...
  ) => Promise<DataArrayPartial> | DataArrayPartial;
  searchFilter?: (option: Option, search: string) => boolean;
  addable?: (
    value: string,
  ) => Promise<OptionOptional | string> | OptionOptional | string | false | null | undefined;
...
export default class NextSelect {
  ...
  constructor(config: Config) {
    ...
    // If select already has a next select id on it lets destroy it first
    ...
    for (const key in config.events) {
      ...
      if (debounceEvents.indexOf(key) !== -1) {
        ...
        (this.events as { [key: string]: any })[key] = (config.events as { [key: string]: any })[
          key
        ];
      }
    }
    ...
    // Add NextSelect to select element
    (this.selectEl as any).next = this;
  }
